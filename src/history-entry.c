...
#define GPHPEDIT_HISTORY_ENTRY_HISTORY_LENGTH_DEFAULT 10

#define GPHPEDIT_HISTORY_ENTRY_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), \
						GPHPEDIT_TYPE_HISTORY_ENTRY, \
						GphpeditHistoryEntryPrivate))

struct _GphpeditHistoryEntryPrivate
{
	gchar              *history_id;
...
static void
gphpedit_history_entry_set_property (GObject      *object,
				  guint         prop_id,
				  const GValue *value,
				  GParamSpec   *spec)
{
	...
	g_return_if_fail (GPHPEDIT_IS_HISTORY_ENTRY (object));

	entry = GPHPEDIT_HISTORY_ENTRY (object);
	...
	case PROP_HISTORY_LENGTH:
		gphpedit_history_entry_set_history_length (entry,
...
static void
gphpedit_history_entry_get_property (GObject    *object,
				  guint       prop_id,
				  GValue     *value,
				  GParamSpec *spec)
{
	...
	priv = GPHPEDIT_HISTORY_ENTRY (object)->priv;
...
static void
gphpedit_history_entry_destroy (GtkObject *object)
{
	gphpedit_history_entry_set_enable_completion (GPHPEDIT_HISTORY_ENTRY (object),
						   FALSE);

	GTK_OBJECT_CLASS (gphpedit_history_entry_parent_class)->destroy (object);
}

static void
gphpedit_history_entry_finalize (GObject *object)
{
	GphpeditHistoryEntryPrivate *priv;
...
static void 
gphpedit_history_entry_class_init (GphpeditHistoryEntryClass *klass)
{
	GObjectClass   *object_class = G_OBJECT_CLASS (klass);
	GtkObjectClass *gtkobject_class = GTK_OBJECT_CLASS (klass);
	
	object_class->set_property = gphpedit_history_entry_set_property;
	object_class->get_property = gphpedit_history_entry_get_property;
	object_class->finalize = gphpedit_history_entry_finalize;
	gtkobject_class->destroy = gphpedit_history_entry_destroy;
	...
	g_object_class_install_property (object_class,
					 ...
					 g_param_spec_uint ("history-length",
							    ...
							    0,
							    G_MAXUINT,
							    GPHPEDIT_HISTORY_ENTRY_HISTORY_LENGTH_DEFAULT,
							    G_PARAM_READWRITE |
							    G_PARAM_STATIC_STRINGS));

	/* TODO: Add enable-completion property */

	g_type_class_add_private (object_class, sizeof(GphpeditHistoryEntryPrivate));
}

static GtkListStore *
get_history_store (GphpeditHistoryEntry *entry)
{
	GtkTreeModel *store;
...
static void
insert_history_item (GphpeditHistoryEntry *entry,
		     const gchar       *text,
		     gboolean           prepend)
{
	GtkListStore *store;
	...
	gphpedit_history_entry_save_history (entry);
}

void
gphpedit_history_entry_prepend_text (GphpeditHistoryEntry *entry,
				  const gchar       *text)
{
	g_return_if_fail (GPHPEDIT_IS_HISTORY_ENTRY (entry));
...
void
gphpedit_history_entry_append_text (GphpeditHistoryEntry *entry,
				 const gchar       *text)
{
	g_return_if_fail (GPHPEDIT_IS_HISTORY_ENTRY (entry));
	g_return_if_fail (text != NULL);
...
void
gphpedit_history_entry_clear (GphpeditHistoryEntry *entry)
{
	GtkListStore *store;
...
static void
gphpedit_history_entry_init (GphpeditHistoryEntry *entry)
{
	GphpeditHistoryEntryPrivate *priv;

	priv = GPHPEDIT_HISTORY_ENTRY_GET_PRIVATE (entry);
	entry->priv = priv;

	priv->history_id = NULL;
	priv->history_length = GPHPEDIT_HISTORY_ENTRY_HISTORY_LENGTH_DEFAULT;
...
void
gphpedit_history_entry_set_history_length (GphpeditHistoryEntry *entry,
					guint              history_length)
{
	g_return_if_fail (GPHPEDIT_IS_HISTORY_ENTRY (entry));
...
guint
gphpedit_history_entry_get_history_length (GphpeditHistoryEntry *entry)
{
	g_return_val_if_fail (GPHPEDIT_IS_HISTORY_ENTRY (entry), 0);

	return entry->priv->history_length;
}

gchar *
gphpedit_history_entry_get_history_id (GphpeditHistoryEntry *entry)
{
	g_return_val_if_fail (GPHPEDIT_IS_HISTORY_ENTRY (entry), NULL);

	return g_strdup (entry->priv->history_id);
}

void
gphpedit_history_entry_set_enable_completion (GphpeditHistoryEntry *entry,
					   gboolean           enable)
{
	g_return_if_fail (GPHPEDIT_IS_HISTORY_ENTRY (entry));
	...
	if (enable)
	{
		...
		gtk_entry_set_completion (GTK_ENTRY (gphpedit_history_entry_get_entry(entry)), 
...
gboolean
gphpedit_history_entry_get_enable_completion (GphpeditHistoryEntry *entry)
{
	g_return_val_if_fail (GPHPEDIT_IS_HISTORY_ENTRY (entry), FALSE);
	
	return entry->priv->completion != NULL;
}

GtkWidget *
gphpedit_history_entry_new (const gchar *history_id,
			 gboolean     enable_completion)
{
	 ...
	 * data in the history through gphpedit_history_entry_
	 * functions.
	 */

	store = gtk_list_store_new (1, G_TYPE_STRING);

	ret = g_object_new (GPHPEDIT_TYPE_HISTORY_ENTRY,
	 ...
	 */
	gphpedit_history_entry_load_history (GPHPEDIT_HISTORY_ENTRY (ret));

	gphpedit_history_entry_set_enable_completion (GPHPEDIT_HISTORY_ENTRY (ret),
						   enable_completion);
 ...
 * simply make the GphpeditHistoryEntry widget implement the 
 * GtkEditable interface. Unfortunately both GtkEditable and
 * GtkComboBox have a "changed" signal and I am not sure how to
 * handle the conflict.
 */
GtkWidget *
gphpedit_history_entry_get_entry (GphpeditHistoryEntry *entry)
{
	g_return_val_if_fail (GPHPEDIT_IS_HISTORY_ENTRY (entry), NULL);
...
void
gphpedit_history_entry_set_escape_func (GphpeditHistoryEntry           *entry,
				     GphpeditHistoryEntryEscapeFunc  escape_func)
{
	GList *cells;
