...
gchar *php_special_keywords[] ={"$GLOBALS","$HTTP_POST_VARS","$THIS","$_COOKIE","$_ENV","$_FILES","$_GET","$_POST","$_REQUEST","$_SERVER","$_SESSION",NULL};
...
void autocomplete_php_variables(GtkWidget *scintilla, gint wordStart, gint wordEnd){
  ...
  buffer = gtk_scintilla_get_text_range (GTK_SCINTILLA(scintilla), wordStart, wordEnd, &length);
  int i=0;
  while (php_special_keywords[i]!=0){
      if (g_str_has_prefix(php_special_keywords[i],buffer)){
        if (!result) {
        result = g_string_new(php_special_keywords[i]);
        result = g_string_append(result, "?3");
        } else {
        result = g_string_append(result, " ");
        result = g_string_append(result, php_special_keywords[i]);
        result = g_string_append(result, "?3");
        }
      }
  i++;
